...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        self._buf_user_dense_as_int = np.zeros(
            (B, self.user_dense_as_int_schema.total_dim), dtype=np.float32)
        self._buf_user_dense = np.zeros((B, self.user_dense_schema.total_dim), dtype=np.float32)
        self._buf_seq = {}
        self._buf_seq_tb = {}
        self._buf_seq_time_features = {}
        self._buf_seq_lens = {}
        for domain in self.seq_domains:
            ...
            self._buf_seq_time_features[domain] = np.zeros((B, max_len, 6), dtype=np.float32)
        ...
        self._user_dense_as_int_plan = []
        offset = 0
        for fid, dim in self._user_dense_as_int_cols:
            ci = self._col_idx.get(f'user_dense_feats_{fid}')
            self._user_dense_as_int_plan.append((ci, dim, offset))
            offset += dim
    ...
    def _load_schema(self, schema_path: str, seq_max_lens: Dict[str, int]) -> None:
        ...
        # ---- user_dense: [[fid, dim], ...] ----
        # Some dense fids should be projected together with user_int features
        # in the NS tokenizer instead of the standalone user_dense token.
        raw_user_dense: List[List[int]] = raw['user_dense']
        self._user_dense_as_int_cols = [
            col for col in raw_user_dense if int(col[0]) in USER_DENSE_AS_INT_FIDS
        ]
        self._user_dense_cols: List[List[int]] = [
            col for col in raw_user_dense if int(col[0]) not in USER_DENSE_AS_INT_FIDS
        ]
        self.user_dense_as_int_schema: FeatureSchema = FeatureSchema()
        for fid, dim in self._user_dense_as_int_cols:
            self.user_dense_as_int_schema.add(fid, dim)
    ...
    @staticmethod
    def _build_time_cyclic_features(
        timestamps: "npt.NDArray[np.int64]",
    ) -> "npt.NDArray[np.float32]":
        """Build cyclic hour/day/week features from unix timestamps.

        Feature order:
        ``[hour_sin, hour_cos, day_sin, day_cos, week_sin, week_cos]``.
        Here ``day`` means day-of-week and ``week`` means week-of-month.
        Zero timestamps are treated as padding and remain all-zero.
        """
        features = np.zeros(timestamps.shape + (6,), dtype=np.float32)
        valid = timestamps > 0
        if not np.any(valid):
            return features

        ts_valid = timestamps[valid]
        dt_valid = ts_valid.astype('datetime64[s]')

        days = dt_valid.astype('datetime64[D]')
        hours = (dt_valid - days).astype('timedelta64[h]').astype(np.int64)

        week_starts = days.astype('datetime64[W]')
        weekdays = (days - week_starts).astype(np.int64)

        month_starts = dt_valid.astype('datetime64[M]')
        day_of_month = (days - month_starts).astype(np.int64) + 1
        week_of_month = np.clip((day_of_month - 1) // 7, 0, 4)

        hour_angle = hours.astype(np.float32) * (2.0 * np.pi / 24.0)
        day_angle = weekdays.astype(np.float32) * (2.0 * np.pi / 7.0)
        week_angle = week_of_month.astype(np.float32) * (2.0 * np.pi / 5.0)

        features_valid = np.stack(
            [
                np.sin(hour_angle),
                np.cos(hour_angle),
                np.sin(day_angle),
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        # ---- user_dense projected with user_int ----
        user_dense_as_int = self._buf_user_dense_as_int[:B]
        user_dense_as_int[:] = 0
        for ci, dim, offset in self._user_dense_as_int_plan:
            col = batch.column(ci)
            padded = self._pad_varlen_float_column(col, dim, B)
            user_dense_as_int[:, offset:offset + dim] = padded

        # ---- remaining user_dense ----
        ...
        result = {
            ...
            'user_dense_as_int_feats': torch.from_numpy(user_dense_as_int.copy()),
        ...
        for domain in self.seq_domains:
            ...
            time_features = self._buf_seq_time_features[domain][:B]
            time_bucket[:] = 0
            time_features[:] = 0
            if ts_ci is not None:
                ...
                time_diff[ts_padded == 0] = 0
                time_features[:] = self._build_time_cyclic_features(ts_padded)
            ...
            result[f'{domain}_time_features'] = torch.from_numpy(time_features.copy())
