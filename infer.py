...
from dataset import FeatureSchema, PCVRParquetDataset, NUM_TIME_BUCKETS, TIME_FEATURE_DIM
...
def resolve_model_cfg(train_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key in _MODEL_CFG_KEYS:
        ...
        if key == 'context_time_dim':
            if 'context_time_dim' in train_config:
                cfg[key] = train_config['context_time_dim']
            elif 'use_context_time_feats' in train_config:
                cfg[key] = TIME_FEATURE_DIM if train_config['use_context_time_feats'] else 0
            else:
                cfg[key] = _FALLBACK_MODEL_CFG[key]
            continue

        if key == 'seq_abs_time_dim':
            if 'seq_abs_time_dim' in train_config:
                cfg[key] = train_config['seq_abs_time_dim']
            elif 'use_seq_abs_time_feats' in train_config:
                cfg[key] = TIME_FEATURE_DIM if train_config['use_seq_abs_time_feats'] else 0
            else:
                cfg[key] = _FALLBACK_MODEL_CFG[key]
            continue
...
def _batch_to_model_input(
    ...
    for domain in seq_domains:
        ...
        seq_abs_time_feats[domain] = device_batch.get(
            ...
            torch.zeros(B, L, 0, dtype=torch.float32, device=device))
    ...
    return ModelInput(
        ...
        context_time_feats=device_batch.get(
            'context_time_feats',
